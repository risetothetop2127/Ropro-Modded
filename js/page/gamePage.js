...
function fetchFeaturedToys(placeId) {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "GetURL", url:"https://api.ropro.io/getFeaturedToys.php?placeid=" + parseInt(placeId)},
			function(data) {
					resolve(data)
			})
	})
...
function addCloudPlayButton() {
	...
	var cloudPlayButtonInterval1 = setInterval(async function() {
		if (document.getElementsByClassName('game-favorite-button-container').length > 0) {
			clearInterval(cloudPlayButtonInterval1)
			var div = document.createElement('div')
			div.innerHTML = `<li class="game-favorite-button-container ropro-cloud-play-container" style="text-align:center;margin-left:-5px;overflow:visible;">
				<div class="tooltip-container" style="position:relative;">
					<div class="follow-button ropro-cloud-play-button">
						<a>
						<img class="ropro-cloud-play-icon-active" src="${chrome.runtime.getURL('/images/cloud_play_active.svg')}" style="height:23px;margin-bottom:6px;margin-top:3px;margin-left:-5px;">
						<img class="ropro-cloud-play-icon-inactive" src="${chrome.runtime.getURL('/images/cloud_play_inactive.svg')}" style="height:23px;margin-bottom:6px;margin-top:3px;margin-left:-5px;filter:invert(0);">
							<div class="icon-label" style="margin-left:0px;">Cloud Play</div>
						</a>
					</div>
					<div style="z-index:1000;background-color:${theme == "dark" ? '#191B1D' : '#DEE1E3'};position:absolute;bottom:-175px;left:-70px;padding:10px;width:auto;border-radius:10px;" class="ropro-cloud-play-tooltip">
					<div style="font-size:14px;margin:auto;font-weight:600;margin-bottom:3px;width:180px;">RoPro Cloud Play</div>
					<p style="font-size:12px;width:auto;height:auto;white-space: normal;text-align:left;">• Launch this game in the cloud via now.gg.<br>• Cloud Play is rendered through servers in the cloud, allowing any device to play Roblox in full graphics mode.</p>
					</div>
				</div>
			</li>`
			document.getElementsByClassName('game-follow-button-container')[0].parentNode.insertBefore(div.childNodes[0], document.getElementsByClassName('game-follow-button-container')[0].nextElementSibling)
			$('.game-favorite-button-container').css("width", "calc(((100% - 160px) / 2) - 1px)")
			$('.game-follow-button-container').css("width", "calc(((100% - 160px) / 2) - 1px)")
			var button = document.getElementsByClassName('ropro-cloud-play-button')[0]
			button.addEventListener('click', async function() {
				if (this.classList.contains('active')) {
					settings = await getStorage("rpSettings")
					this.classList.remove('active')
					document.body.classList.remove("ropro-cloud-play-activated")
					settings["cloudPlayActive"] = false
					...
					settings["cloudPlayActive"] = true
					setStorage("rpSettings", settings)
					cloudPlayActive = true
				}
			})
			if (await fetchSetting('cloudPlayActive')) {
...
async function addFeaturedToys() {
	var recommendedInterval = setInterval(async function() {
		if (document.getElementById('recommended-games-container') != null) {
			clearInterval(recommendedInterval)
			recommendedContainer = document.getElementById('recommended-games-container')
			var div = document.createElement('div')
			div.innerHTML = `<div class="container-list games-detail" style="margin-top:20px;"><h2>Featured Toys <div style="display:inline-block;font-size:8px;vertical-align: top;margin-top:4px;margin-left:-3px;">[BETA]</div></h2><div id="featured-toys-carousel" class="game-carousel featured-toys-carousel" style="margin-bottom:10px;max-height:350px;min-height:100px;"><p style="margin:auto;margin-top:25px;">No Featured Toys found for this game.</p></div><div style="padding:10px;display:inline-block;background-color:${theme == "dark" ? '#191B1D' : '#DEE1E3'};width:auto;border-radius:10px;padding-right:20px;"><span class="icon-moreinfo" style="float:left;margin-top:13px;margin-left:0px;" title="RoPro Featured Toys Info"></span><p style="font-size:12px;margin-top:0px;margin-left:10px;float:right;">Official Roblox &amp; developer toys for this game. RoPro earns an Amazon affiliate commission on sales.<br>Featured Toys only appear for <b>13+</b> Roblox users. You can disable Featured Toys in the RoPro settings.<br>Are you the developer of this game? Email your official toys to <b>devs@ropro.io</b> to have them featured.</p></div></div>`
			recommendedContainer.parentNode.insertBefore(div.childNodes[0], recommendedContainer)
			toys = await fetchFeaturedToys(globalGameId)
			if (toys.length > 0) {
				document.getElementById('featured-toys-carousel').innerHTML = `<button style="margin:auto;" class="btn-growth-md btn-secondary-md"><a href="https://ropro.io/toys/${parseInt(globalGameId)}" target="_blank">View ${toys.length} Featured Toys on ropro.io</a></button>`
			}
			/**for (i = 0; i < toys.length; i++) {
				document.getElementById('featured-toys-carousel').innerHTML += `<a title="${stripTags(toys[i]['name'])}" style="margin-right:10px;width:170px;" href="${stripTags(toys[i]['link'])}" target="_blank"><div style="background-color:#383B3D;padding:10px;border-radius:10px;width:170px;"><img class="featured-toy-image" style="width:150px;" src="${stripTags(toys[i]['icon'])}"></div><h5 style="padding-bottom:0px;">${stripTags(toys[i]['name'])}</h5></a>`
			}**/
		}
	}, 250)
}
...
async function checkGamePage() {
	...
	if (typeof gameSplit != 'undefined') {
		...
		if (isNormalInteger(globalGameId)) { // Valid Game Page
			...
			if (await fetchSetting("featuredToys")) {
				addFeaturedToys()
			}
		}
	}
