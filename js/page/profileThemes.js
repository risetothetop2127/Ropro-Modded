...
function fetchThemesV2() {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "GetURL", url:"https://api.ropro.io/getThemesV2.json"},
			function(data) {
				resolve(data)
			}
		)
		...
		if (await checkVerification()) {
			if (this.innerHTML == chrome.i18n.getMessage("SaveTheme")) {
				if (myTheme != null && myTheme.hasOwnProperty('version') && myTheme['version'] == 2) {
					...
					response = await saveTheme(userID, myTheme['name'], 2)
...
function addThemesButton() {
	...
	detailsAction.innerHTML += `<li><button id="editThemes" class="btn-control-md">Edit Theme</button></li>`
	document.getElementById('editThemes').addEventListener('click', function() {
		if (this.innerText == "Edit Theme") {
			this.innerText = "Close Themes"
			loadThemesV2()
		} else {
			this.innerText = "Edit Theme"
			document.getElementById('profileThemesSelectorBox').remove()
		}
	})
}
